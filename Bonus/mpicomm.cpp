...
// function that print the final topology
...
int main (int argc, char *argv[]) {
    ...
    if (rank == COORD0) {
        ...
        MPI_Recv(&sizeCOORD1, 1, MPI_INT, COORD2, 1, MPI_COMM_WORLD, &status);
        int topologyCOORD1[sizeCOORD1];
        MPI_Recv(&topologyCOORD1, sizeCOORD1, MPI_INT, COORD2, 1, MPI_COMM_WORLD, &status);
...
//
        ...
        // send tasks for the second coordinator through the third coordinator
        ...
        // send tasks for the third coordinator
        ...
        MPI_Recv(&sizeCOORD0, 1, MPI_INT, COORD2, 1, MPI_COMM_WORLD, &status);
        int topologyCOORD0[sizeCOORD0];
        MPI_Recv(&topologyCOORD0, sizeCOORD0, MPI_INT, COORD2, 1, MPI_COMM_WORLD, &status);
        ...
        int numberOfTasks;
        MPI_Recv(&numberOfTasks, 1, MPI_INT, COORD2, 1, MPI_COMM_WORLD, &status);
        int tasks[numberOfTasks];

        // receive the tasks from the third coordinator and send them to workers, than send back
        // the results to the third coordinator
        for(int i = 1; i < sizeCOORD1; i ++) {
            MPI_Recv(&tasks, numberOfTasks, MPI_INT, COORD2, 1, MPI_COMM_WORLD, &status);
            ...
            MPI_Send(&tasks, numberOfTasks, MPI_INT, COORD2, 1, MPI_COMM_WORLD);
            printf("M(%d,%d)\n", rank, COORD2);
        }
        ...
        MPI_Send(&numberOfTasks, 1, MPI_INT, COORD1, 1, MPI_COMM_WORLD);
        printf("M(%d,%d)\n", rank, COORD1);

        // receive tasks from the first coordinator and send them to the second coordinator
        // than receive the results and send them back to the first coordinator
        for(int i = 1; i < sizeCOORD1; i ++) {
            MPI_Recv(&tasks, numberOfTasks, MPI_INT, COORD0, 1, MPI_COMM_WORLD, &status);

            MPI_Send(&tasks, numberOfTasks, MPI_INT, COORD1, 1, MPI_COMM_WORLD);
            printf("M(%d,%d)\n", rank, COORD1);

            MPI_Recv(&tasks, numberOfTasks, MPI_INT, COORD1, 1, MPI_COMM_WORLD, &status);
